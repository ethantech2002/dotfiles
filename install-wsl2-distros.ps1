...
Write-Host ""
Write-Host "Available distributions from WSL online store:" -ForegroundColor Cyan
wsl --list --online
Write-Host ""
...
# Install Arch Linux from GitHub (ArchWSL by yuk7)
...
try {
    ...
    $archDir = "$env:LOCALAPPDATA\ArchWSL"
    
    Write-Host "  Downloading ArchWSL from GitHub..." -ForegroundColor Yellow
    Invoke-WebRequest -Uri $archUrl -OutFile $archZip -UseBasicParsing
    
    # Extract to local directory
    Write-Host "  Extracting ArchWSL..." -ForegroundColor Yellow
    if (Test-Path $archDir) {
        Remove-Item -Path $archDir -Recurse -Force
    }
    Expand-Archive -Path $archZip -DestinationPath $archDir -Force
    
    # Install ArchWSL
    Write-Host "  Installing ArchWSL..." -ForegroundColor Yellow
    Set-Location $archDir
    & .\Arch.exe install
    
    # Register with WSL
    Write-Host "  Registering with WSL..." -ForegroundColor Yellow
    
    # Clean up
    Remove-Item -Path $archZip -Force
    ...
    Write-Host "  You can manually install from: https://github.com/yuk7/ArchWSL" -ForegroundColor Yellow
}
...
Write-Host "   Run: wsl -d Ubuntu" -ForegroundColor Yellow
...
Write-Host "   Create username and password when prompted" -ForegroundColor Gray
Write-Host "   Note: First launch will initialize the system" -ForegroundColor Gray
Write-Host ""
